...
{
    public class ShiftClickIntoSlot : ModPlayer
    {
        ...
        private bool CanShiftClick(UpgradeTemplate item, UpgradeableItemTemplate parentItem)
        {
            ...
            for (int i = 0; i < parentItem.Upgrades.Length - 1; i++)
            {
                // First of all, check for duplicates
                if (parentItem.Upgrades[i] == item.Type)
                {
                    duplicates = true;
                }
                if (parentItem.Upgrades[i] == 0)
                {
                    flag = true;
                }
        ...
        private void ShiftClickInventory(UpgradeTemplate item, UpgradeableItemTemplate parentItem)
        {
            if (item.IsOverclock)
            {
                if (parentItem.Overclock == 0)
                {
                    parentItem.Upgrades[^1] = item.Type;
                }
            }
            else
            {
                for (int i = 0; i < parentItem.Upgrades.Length; i++)
                {
                    if (parentItem.Upgrades[i] == 0)
                    {
                        parentItem.Upgrades[i] = item.Type;
                        break;
                    }
                }
            }
